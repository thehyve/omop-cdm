...
class FactRelationship(Base):
    ...
    domain_concept_id_1: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID), index=True)
    fact_id_1: Mapped[int] = mapped_column(Integer)
    domain_concept_id_2: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID), index=True)
    fact_id_2: Mapped[int] = mapped_column(Integer)
    relationship_concept_id: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID), index=True)
...
class Specimen(Base):
    ...
    specimen_date: Mapped[Optional[datetime.date]] = mapped_column(Date)
    specimen_datetime: Mapped[datetime.datetime] = mapped_column(DateTime)
    ...
    anatomic_site_concept_id: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID))
    disease_status_concept_id: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID))
...
class VisitDetail(Base):
    ...
    visit_detail_source_concept_id: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID))
    discharge_to_source_value: Mapped[Optional[str]] = mapped_column(String(50))
    preceding_visit_detail_id: Mapped[Optional[int]] = mapped_column(ForeignKey(FK_VISIT_DETAIL_ID))
    visit_detail_parent_id: Mapped[Optional[int]] = mapped_column(ForeignKey(FK_VISIT_DETAIL_ID))
...
class Measurement(Base):
    ...
    measurement_source_concept_id: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID))
    unit_source_value: Mapped[Optional[str]] = mapped_column(String(50))
...
class Observation(Base):
    ...
    observation_source_concept_id: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID))
    unit_source_value: Mapped[Optional[str]] = mapped_column(String(50))
...
class SurveyConduct(Base):
    ...
    visit_occurrence_id: Mapped[Optional[int]] = mapped_column(ForeignKey(FK_VISIT_OCCURRENCE_ID))
    visit_detail_id: Mapped[Optional[int]] = mapped_column(ForeignKey(FK_VISIT_DETAIL_ID))
    response_visit_occurrence_id: Mapped[Optional[int]] = mapped_column(ForeignKey(FK_VISIT_OCCURRENCE_ID))

    assisted_concept: Mapped["Concept"] = relationship("Concept", foreign_keys="SurveyConduct.assisted_concept_id")
    collection_method_concept: Mapped["Concept"] = relationship(
        "Concept", foreign_keys="SurveyConduct.collection_method_concept_id"
    )
    person: Mapped["Person"] = relationship("Person", foreign_keys="SurveyConduct.person_id")
    provider: Mapped["Provider"] = relationship("Provider", foreign_keys="SurveyConduct.provider_id")
    ...
    survey_concept: Mapped["Concept"] = relationship("Concept", foreign_keys="SurveyConduct.survey_concept_id")
    survey_source_concept: Mapped["Concept"] = relationship(
        "Concept", foreign_keys="SurveyConduct.survey_source_concept_id"
    )
    timing_concept: Mapped["Concept"] = relationship("Concept", foreign_keys="SurveyConduct.timing_concept_id")
    validated_survey_concept: Mapped["Concept"] = relationship(
        "Concept", foreign_keys="SurveyConduct.validated_survey_concept_id"
    )
    visit_detail: Mapped["VisitDetail"] = relationship("VisitDetail", foreign_keys="SurveyConduct.visit_detail_id")
    visit_occurrence: Mapped["VisitOccurrence"] = relationship(
...
class NoteNlp(Base):
    ...
    note_id: Mapped[int] = mapped_column(ForeignKey(f"{CDM_SCHEMA}.note.note_id"), index=True)
    ...
    note_nlp_concept_id: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID), index=True)
...
class Cost(Base):
    ...
    cost_concept: Mapped["Concept"] = relationship("Concept", foreign_keys="Cost.cost_concept_id")
    cost_source_concept: Mapped["Concept"] = relationship("Concept", foreign_keys="Cost.cost_source_concept_id")
...
class PayerPlanPeriod(Base):
    ...
    stop_reason_source_concept_id: Mapped[int] = mapped_column(ForeignKey(FK_CONCEPT_ID))

    contract_concept: Mapped["Concept"] = relationship("Concept", foreign_keys="PayerPlanPeriod.contract_concept_id")
    contract_person: Mapped["Person"] = relationship("Person", foreign_keys="PayerPlanPeriod.contract_person_id")
...
class Location(Base):
    ...
    region_concept: Mapped["Concept"] = relationship("Concept", foreign_keys="Location.region_concept_id")
...
class LocationHistory(Base):
    ...
    location: Mapped["Location"] = relationship("Location", foreign_keys="LocationHistory.location_id")
...
class Metadata(Base):
    ...
    metadata_concept_id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
...
class Concept(Base):
    ...
    __table_args__ = (
        CheckConstraint("(COALESCE(invalid_reason,'D') in ('D','U'))", name="chk_c_invalid_reason"),
        CheckConstraint("(COALESCE(standard_concept,'C') in ('C','S'))", name="chk_c_standard_concept"),
